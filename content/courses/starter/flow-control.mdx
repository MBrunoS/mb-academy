...
<Callout type="warn">
É importante lembrar do `break` ao final de cada `case`, pois sem ele, o JavaScript **continua executando os próximos blocos**, mesmo que a condição tenha sido satisfeita — comportamento chamado de “fall-through”.
</Callout>
...
1. Crie uma variável `nota` e atribua a ela um valor numérico (de 0 a 10).
...
3. Teste com diferentes valores para a variável `nota` e veja se o resultado está correto.
...
Com o controle de fluxo, seu código passa a **pensar e se adaptar**. Ele pode reagir a diferentes valores, executar blocos diferentes de acordo com regras, e até combinar várias condições em uma lógica sofisticada. Esse é um dos pilares da programação: a capacidade de tomar decisões e conduzir o programa por caminhos diferentes.

Essa seção fecha o ciclo de manipulação e decisão. Primeiro, aprendemos a armazenar dados. Agora, aprendemos a reagir a esses dados. No próximo passo, vamos ver como **repetir ações de forma controlada**, usando as estruturas de repetição como `for`, `while` e `do-while`.

Elas nos permitirão processar coleções de dados, repetir cálculos, e muito mais — de forma simples, legível e eficiente.