...
Você concluiu os seis capítulos obrigatórios do curso: construiu, testou e publicou o Dashboard de Finanças Pessoais com uma URL real. Isso já é suficiente para avançar para o Pleno sempre que quiser.

Antes de seguir, porém, vale passar pela **trilha complementar**: cinco capítulos opcionais — mas recomendados — que não ensinam mais sintaxe, e sim o que separa um júnior que só "faz funcionar" de um júnior em quem o time confia para crescer. Ao longo dela, você vai ver como deixar seu código mais fácil de manter, como pensar visualmente em uma interface, como o trabalho em si é organizado dentro de um time de verdade, como usar IA de forma responsável no dia a dia (não só nos exercícios) e, por fim, como transformar tudo isso em uma candidatura competitiva para sua primeira vaga. Nenhum desses capítulos bloqueia seu avanço de nível — você pode pular direto para o Pleno sem cursá-los —, mas cada um cobre uma lacuna real que separa quem só sabe programar de quem está pronto para o dia a dia de um time.

Vamos começar pelo próprio código. Fazer o código funcionar é só a primeira parte do trabalho. A segunda parte — que separa quem está começando de quem já tem experiência — é fazer com que esse código continue fácil de entender e de mudar daqui a um mês, quando você (ou outra pessoa do time) precisar mexer nele de novo.
