...
<Callout>
  A URL `https://api.exemplo.dev/gastos` é fictícia — usada aqui só para fins didáticos. Mas o código acima é **exatamente** o que você escreveria contra uma API real: o padrão de `fetch` + `.json()` não muda dependendo do servidor do outro lado.
</Callout>
...
```ts
const resposta = await fetch("https://api.exemplo.dev/gastos/999999");
...
E uma API fictícia que responde em `https://api.exemplo.dev/gastos/:id`, devolvendo um JSON no formato:
...
2. Ao clicar no botão, leia o valor digitado em `#input-id` e faça uma requisição `fetch` para `https://api.exemplo.dev/gastos/{id}`.
