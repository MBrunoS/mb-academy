...
async function login(email: string, senha: string): Promise<string> {
  const resposta = await fetch("https://api.exemplo.dev/auth/login", {
    method: "POST",
...
1. Implemente uma função `login(email: string, senha: string): Promise<string>` que faça `POST` para `"https://api.exemplo.dev/auth/login"` e retorne o token recebido, lançando um erro com a mensagem `"E-mail ou senha incorretos."` especificamente quando o status for `401`.
