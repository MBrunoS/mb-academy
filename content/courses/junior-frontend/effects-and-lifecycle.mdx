...
1. Crie um componente `PainelFinanceiro` com estado `gastos: Gasto[]`, inicializado lendo o valor salvo no `localStorage` sob a chave `"mbacademy:gastos"` — use a forma de função no `useState` (`useState(() => ...)`) para ler e fazer `JSON.parse` do valor salvo, ou retornar um array vazio se não houver nada salvo ainda.
